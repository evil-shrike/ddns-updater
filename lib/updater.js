 ...
 * @property {String} service Dynamic DNS Service name
...
/**
 * @param {Object} config
 * @param {Array<DomainInfo>} config.domains Array of domain registration info
 * @param {String} [config.interval]
 * @param {Boolean} [config.updateAlways]
 * @param {String} [config.resolver] Resolver to use for resolve public IP
 * @param {Object} [config.updaters] a mapping of service name to updater class
 */
function DDNSUpdater(config) {
    ...
    config.domains.forEach(function (domain) {
        ...
        if (!that.updaters[domain.service]) {
            var Updater
            if (config.updaters && config.updaters[domain.service]) {
                Updater = config.updaters[domain.service];
            } else {
                Updater = require("../updaters/" + domain.service + ".js");
            }
